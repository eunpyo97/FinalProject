...
class User(db.Model):
    ...
    user_id = db.Column(db.String(36), unique=True, nullable=False, default=lambda: str(uuid.uuid4()))  # UUID 자동 생성
    ...
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # 계정 생성 시간
    updated_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))  # 마지막 수정 시간
...
class UserProfile(db.Model):
    ...
    user_id = db.Column(db.String(36), db.ForeignKey('users.user_id'), nullable=False, unique=True)  # User와 1:1 관계
    nickname = db.Column(db.String(80), unique=True, nullable=False)
    profile_image = db.Column(db.String(200), nullable=True, default='/static/default_profile.jpg')  # 프로필 사진
...
class UserToken(db.Model):
    ...
    token_type = db.Column(db.String(50), nullable=False)  # 토큰 유형 ('refresh', 'password_reset')
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # 생성 시간
...
class UserLog(db.Model):
    ...
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # 작업 발생 시간
...
class UserTime(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, unique=True)  # User와 1:1 관계
    created_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc))  # 계정 생성 시간
    updated_at = db.Column(db.DateTime, default=lambda: datetime.now(timezone.utc), onupdate=lambda: datetime.now(timezone.utc))  # 마지막 수정 시간
