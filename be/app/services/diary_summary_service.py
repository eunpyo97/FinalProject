...
def generate_summary(chatroom_id):
    ...
    # MongoDB에서 대화방 데이터 조회
    try:
        chatroom = mongo.db.chatrooms.find_one(
            {"chatroom_id": chatroom_id},
            {"chats": 1, "conversation_end": 1}  # 필요한 필드만 선택
        )
        if not chatroom:
            logging.warning(f"[WARNING] 대화방 ID: {chatroom_id} - 해당 대화방을 찾을 수 없음")
            return {"error": "해당 대화방을 찾을 수 없습니다."}, 404
        
        if not chatroom.get("conversation_end"):
            logging.warning(f"[WARNING] 대화방 ID: {chatroom_id} - 대화가 종료되지 않음")
            return {"error": "대화가 종료되지 않았습니다."}, 400
        
        # 대화 내용 합치기
        conversation_text = ""
        for chat in chatroom.get("chats", []):  # chats 배열이 없는 경우 빈 리스트로 처리
            user_message = chat.get("user_message", "").strip()
            bot_response = chat.get("bot_response", "").strip()
            if user_message or bot_response:  # 메시지가 비어있지 않은 경우만 추가
                conversation_text += f"User: {user_message}\nBot: {bot_response}\n"
        
        if not conversation_text.strip():  # 대화 내용이 없는 경우
            logging.warning(f"[WARNING] 대화방 ID: {chatroom_id} - 대화 내용이 없음")
            return {"error": "대화 내용이 없습니다."}, 400
        
        logging.debug(f"[DEBUG] 대화방 ID: {chatroom_id} - 대화 내용: {conversation_text}")
